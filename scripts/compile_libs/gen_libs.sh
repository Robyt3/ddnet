...
(
	# TODO: Switch to building with CMake. https://github.com/xiph/opusfile is already a CMake project but the latest release version isn't yet.
...
function _copy_boringssl() {
	mkdir -p ddnet-libs/boringssl/"$TARGET_PLATFORM"/lib"$2"
	cp compile_libs/boringssl/build_"$TARGET_PLATFORM"_"$1"/libcrypto.a ddnet-libs/boringssl/"$TARGET_PLATFORM"/lib"$2"/libcrypto.a
	cp compile_libs/boringssl/build_"$TARGET_PLATFORM"_"$1"/libssl.a ddnet-libs/boringssl/"$TARGET_PLATFORM"/lib"$2"/libssl.a
}
