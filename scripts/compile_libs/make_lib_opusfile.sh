...
function make_opusfile() {
	...
	(
		...
		if [[ "$_EXISTS_PROJECT" == 0 ]]; then
			...
			cp -R ../../ogg/include .
			cp -R ../../opus/include .
			cp -R ../../ogg/"$1"/include/ogg/* include/ogg/
			cp ../../ogg/"$1"/libogg.a libogg.a
			cp ../../opus/"$1"/libopus.a libopus.a
		fi
...
function compile_all_opusfile() {
	if [[ "${2}" == "android" ]]; then
		make_opusfile build_"$2"_arm armv7a-linux-androideabi "$1" "$2"
		make_opusfile build_"$2"_arm64 aarch64-linux-android "$1" "$2"
		make_opusfile build_"$2"_x86 i686-linux-android "$1" "$2"
		make_opusfile build_"$2"_x86_64 x86_64-linux-android "$1" "$2"
	elif [[ "${2}" == "webasm" ]]; then
		make_opusfile build_"$2"_wasm "" "$1" "$2"
	fi
}
