...
void CFileBrowser::RenderFileDialog()
{
	Ui()->MapScreen();
	...
	if(!m_ShowingRoot)
	{
		...
		if(Editor()->DoButton_Editor(&m_ButtonSortFilenameId, aLabelButtonSortFilename, 0, &ButtonFileName, BUTTONFLAG_LEFT, "Sort by file name."))
		{
...
bool CFileBrowser::IsValidSaveFilename() const
{
	return m_pCurrentPath == m_aCurrentFolder ||
	       (m_pCurrentPath == m_aCurrentLink && str_startswith(m_aCurrentLink, "themes") == 0);
}
...
void CFileBrowser::FilelistPopulate(int StorageType, bool KeepSelection)
{
	...
	else
	{
		...
		{
			if(str_comp(m_aButtonText, "Save") != 0 && Storage()->FolderExists("downloadedmaps", StorageType))
			{
				CFilelistItem Item;
...
CUi::EPopupMenuFunctionResult CFileBrowser::CPopupConfirmDelete::Render(void *pContext, CUIRect View, bool Active)
{
		...
		(Active && pFileBrowser->Ui()->ConsumeHotkey(CUi::HOTKEY_ENTER)))
	{
		...
		{
			if(pFileBrowser->Storage()->RemoveFolder(pConfirmDeleteContext->m_aDeletePath, pFileBrowser->m_StorageType))
			{
		...
		{
			if(pFileBrowser->Storage()->RemoveFile(pConfirmDeleteContext->m_aDeletePath, pFileBrowser->m_StorageType))
			{
				pFileBrowser->FilelistPopulate(pFileBrowser->m_StorageType, true);
			}
			else
...
CUi::EPopupMenuFunctionResult CFileBrowser::CPopupConfirmOverwrite::Render(void *pContext, CUIRect View, bool Active)
{
	...
	if(pEditor->DoButton_Editor(&pConfirmOverwriteContext->m_ButtonOverwriteId, "Overwrite", EditorButtonChecked::DANGEROUS_ACTION, &ButtonOverride, BUTTONFLAG_LEFT, nullptr) ||
		(Active && pFileBrowser->Ui()->ConsumeHotkey(CUi::HOTKEY_ENTER)))
	{
		pFileBrowser->m_pfnOpenCallback(pConfirmOverwriteContext->m_aOverwritePath, pFileBrowser->m_StorageType, pFileBrowser->m_pOpenCallbackUser);
		return CUi::POPUP_CLOSE_CURRENT;
	}
