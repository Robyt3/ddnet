...
void CEnvelopeEditor::Render(CUIRect View)
{
	...
	if(pEnvelope)
	{
		...
		// handle time bar
		{
			...
			ColorRGBA BarColor;
			if(Ui()->CheckActiveItem(&Editor()->m_AnimateTime)) // FIXME: do not use this as UI ID; should be separate per map
			{
				...
				if(m_Operation == EEnvelopeEditorOp::DRAG_TIME_BAR)
				{
					...
					Editor()->m_AnimateTime += DeltaX / Editor()->m_AnimateSpeed;
					Editor()->m_AnimateTime = std::max(Editor()->m_AnimateTime, 0.0f);
				}
			...
			else if(Ui()->HotItem() == &Editor()->m_AnimateTime)
			{
				if(Ui()->MouseButton(0))
				{
					Ui()->SetActiveItem(&Editor()->m_AnimateTime);
			...
			float Time = Editor()->m_AnimateTime * Editor()->m_AnimateSpeed;
