...
CUi::EPopupMenuFunctionResult CEditor::PopupMenuSettings(void *pContext, CUIRect View, bool Active)
{
	...
	View.HSplitTop(12.0f, &Slot, &View);
	{
		...
		static int s_ButtonNo = 0;
		static int s_ButtonYes = 0;
		if(pEditor->DoButton_Ex(&s_ButtonNo, "No", !pEditor->m_ShowEnvelopePreview, &No, BUTTONFLAG_LEFT, "Do not preview path of quads with position envelope.", IGraphics::CORNER_L))
		{
			pEditor->m_ShowEnvelopePreview = false;
			pEditor->m_ActiveEnvelopePreview = EEnvelopePreview::NONE;
		}
		if(pEditor->DoButton_Ex(&s_ButtonYes, "Yes", pEditor->m_ShowEnvelopePreview, &Yes, BUTTONFLAG_LEFT, "Preview path of quads with position envelope.", IGraphics::CORNER_R))
		{
