...
{
	// TODO: maybe add a drop-down menu of all maps on left side to make it easier when there are many tabs
	...
	for(const auto &pMap : m_vpMaps)
	{
		...
		if(m_MapTabsScrollRegion.AddRect(MapButton, m_MapTabsRevealSelected && m_SelectedMap == Index))
		{
			...
			const int TabButtonResult = DoButton_Ex(&pMap->m_TabSelectButtonId, pMap->m_aDisplayName, m_SelectedMap == Index ? 1 : 0, &MapButton, BUTTONFLAG_LEFT | BUTTONFLAG_RIGHT, aTooltip, IGraphics::CORNER_L);
