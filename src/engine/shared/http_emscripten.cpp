...
EM_JS(void, EscapeUrlJsImpl, (char *pBuffer, size_t BufferSize, const char *pUrl), {
	const escapedString = encodeURIComponent(UTF8ToString(pUrl));
	stringToUTF8(escapedString, pBuffer, BufferSize);
});

void EscapeUrl(char *pBuffer, size_t BufferSize, const char *pUrl)
{
	EscapeUrlJsImpl(pBuffer, BufferSize, pUrl);
}
